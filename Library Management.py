...
# creating the object "pandiyarajan" for the class "customer" with some arguments
...
while True:
  ...
  if option == 1:
    ...
    requested_book = pandiyarajan.request_book()
    status = lib_cbe.lend_book(requested_book)
    if status == True:
      pandiyarajan.add_book(requested_book)
  elif option == 3:
    pandiyarajan.show()
  elif option == 4:
    returned_book = pandiyarajan.return_book()
    status=pandiyarajan.lend_book_to_lib(returned_book)
  elif option == 0:
    break
